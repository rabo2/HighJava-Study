...
/*
     ...
     1. 드라이버 로딩 ==> 라이브러리를 사용할 수 있게 메모리로 읽어 들이는 작업
        ...
        ==> 생략하면 getConnetction()메서드에서 자동으로 로딩해준다.
     ...
     4. 결과 처리 ==> 질의 결과를 받아서 원하는 작업을 수행한다.
       ...
       2) SQL문이 select문이 아닐 경우에는 정수값이 반환된다.(insert, update, delete)
...
//      PreparedStatement pstmt = null;
      ...
      try {
         ...
         conn = DriverManager.getConnection(
               "jdbc:oracle:thin:@localhost:1521:xe","KMS96", "java");
         // 3. 질의
         // 3-1 SQL문 작성
         ...
         // 3.3 SQL문 실행(실행할 SQL문이 select문이기 때문에 결과가 ResultSet객체에
         //      저장되어 반환된다.)
         ...
         // ResultSet에 저장된 데이터를 차례로 꺼내오면 반복문과 next()메서드를 이용한다.
         // rs.next() ==> ResultSet객체의 데이터를 가리키는 포인터를
         //        다음번째 레코드 위치로 이동시키고 그곳에 데이터가 있으면
         //          true를반환하고 없으면 false를 반환한다
         while(rs.next()) {
